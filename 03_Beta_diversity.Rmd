...
par(mfrow = c(2,2));plot(west_eu, which = 1:4, type = "pearson", ask=FALSE, main = "west_eu")
plot(west_eu, which = 5, type = "deviance", sub.caption = "", main = "west_eu")
...
plot(central_eu, which = 5, type = "deviance", sub.caption = "", "central_eu")
...
# plot(north_eu, which = 5, type = "deviance", sub.caption = "",  main = "north_eu")
...
plot(east_eu, which = 5, type = "deviance", sub.caption = "", "east_eu")
...
plot(sib, which = 5, type = "deviance", sub.caption = "", main = "sib")
...
plot(kaz, which = 5, type = "deviance", sub.caption = "", "kaz")
...
```{r visualizations, echo=FALSE, dpi = 300, fig.height=5,fig.width=9}
## Visualizations ----
...
p_beta <- rbind(model_parameters(north_eu) %>% as.data.frame() %>% mutate(region = "north_eu"),
      model_parameters(central_eu) %>% as.data.frame() %>% mutate(region = "central_eu"),
      model_parameters(west_eu) %>% as.data.frame()  %>% mutate(region = "west_eu"),
      model_parameters(east_eu) %>% as.data.frame()  %>% mutate(region = "east_eu"),
      model_parameters(sib) %>% as.data.frame()  %>% mutate(region = "sib"),
      model_parameters(jak) %>% as.data.frame()  %>% mutate(region = "jak"),
      model_parameters(kaz) %>% as.data.frame()  %>% mutate(region = "kaz"),
      model_parameters(alp) %>% as.data.frame()  %>% mutate(region = "alps")) %>% 
  mutate(sig = ifelse(p < 0.05, "significant", "not-significant"))
pred_beta <- pred_beta %>% 
  left_join(p_beta, by = c("region", "group" = "Parameter"))
...
p1 <- pred_beta %>%
  ...
  scale_y_continuous("β-diversity", limits = c(0.65,1)) +
...
p2 <- pred_beta %>%
  filter(group == "mean_np") %>% 
    mutate(region = factor(region, levels = c("west_eu","alps","central_eu", "east_eu", 
                                            "north_eu", "kaz", "sib", "jak"))) %>% 
  ggplot(aes(x = exp(x), y = predicted, color = region, fill = region)) +
   geom_ribbon(aes(ymin = conf.low, ymax = conf.high, alpha = sig), color = NA) +
  geom_line(aes(linetype = sig)) +
  ...
  guides(alpha = "none", linetype= "none")
p3 <- coef_beta %>% 
  filter(Parameter %in% c("diss_NP", "diss_ndep", "mean_ndep", "mean_np")) %>% 
  mutate(region = factor(region, levels = c("west_eu","alps","central_eu", "east_eu", 
                                            "north_eu", "kaz", "sib", "jak"))) %>% 
  mutate(Parameter = factor(Parameter, levels = c("diss_NP","mean_np",
                                                  "diss_ndep", "mean_ndep"))) %>% 
  ggplot(aes(x = Std_Coefficient, y = reorder(region, rev(region)), fill = region)) +
  ...
  scale_y_discrete("", labels = region_label2, limits = rev) +
  scale_fill_manual(values = region_color) +
  facet_wrap(~Parameter, ncol = 2, labeller = as_labeller(var_labels)) + 
  theme_bw() +
...
p4 <- (p3|(p2/p1)) + plot_annotation(tag_levels = 'a') + plot_layout(guides = 'collect')
  
ggsave("figures/Beta_np_ndep.png", p4, dpi = 300, height = 5, width = 10)
...
g <- env_data %>% 
  ...
  scale_fill_manual(values = region_color, labels = region_label2) +
  scale_x_discrete("", labels = region_label2) +
  scale_y_continuous("") +
  theme_bw() +
...
```{r map, dpi = 300, fig.height=5, fig.width=9}
...
ggplot() +
  ...
  geom_rect(data = regions[regions$region != "south_eu",] , aes(xmin = xmin, ymin = ymin, xmax = xmax, ymax = ymax), fill = NA,colour = "black") +
...
ggsave("figures/Regions-beta-div.png", dpi = 300, height = 5, width = 9)
